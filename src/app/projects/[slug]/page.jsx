...
import PageBannerProject from '@/src/modules/page_banner/PageBannerProject';
...
export async function generateMetadata({ params: paramsPromise }) {
	...
	try {
		const pageData = await apiService.get(
			`/api/get-req-data/product-data?type=slug&value=${params?.slug}&image=yes&post=yes&file=yes&specification=yes&gallery=yes&variation=yes`,
			{
				cache: true,
				ttl: 10 * 60 * 1000, // 10 minutes cache
			},
		);
		const meta = pageData?.data?.product_data;
...
// Synchronous component for rendering product page content
function PageContent({ data }) {
	...
	return (
		<>
			<PageBannerProject data={data?.data} />
			<RenderedSection product={data} data={data?.data} />
		</>
	);
}
